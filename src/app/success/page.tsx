...
export default function SuccessPage() {
  ...
  const downloadUrl = process.env.NEXT_PUBLIC_DOWNLOAD_PAGE_URL || 'https://jhericojohnbalasa.systeme.io/fundementals-truth';

  useEffect(() => {
    const timer = setInterval(() => {
      setCountdown((c) => {
        if (c <= 1) { clearInterval(timer); window.location.href = downloadUrl; return 0; }
        return c - 1;
      });
    }, 1000);
  ...
  return (
    <main className="min-h-screen bg-radial-glow flex items-center justify-center px-4">
      <div className="glass-card p-10 max-w-md w-full text-center" style={{ borderColor: 'rgba(0,255,135,0.4)' }}>
        ...
        <h1 className="text-4xl font-bold text-white mb-2" style={{ fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
          Payment Verified! 🎉
        </h1>
        <p className="text-[var(--green)] font-semibold mb-4">GCash Receipt Confirmed</p>
        <p className="text-gray-300 text-sm mb-6">
          Redirecting to your download page in{' '}
          <span className="text-[var(--green)] font-bold text-lg">{countdown}</span> seconds.
        </p>
        <div className="w-full h-1.5 bg-[var(--border)] rounded-full overflow-hidden mb-6">
          <div className="h-full bg-[var(--green)] rounded-full transition-all duration-1000"
            style={{ width: `${((5 - countdown) / 5) * 100}%` }} />
        </div>
        <a href={downloadUrl}
          className="btn-shimmer inline-flex items-center gap-2 px-8 py-3 rounded-xl font-bold text-black mb-4">
          <Download size={18} /> Go to Download Page Now
        </a>
        <div className="mt-4 pt-4 border-t border-[var(--border)]">
          <div className="flex items-center justify-center gap-2 text-xs text-gray-500">
            <BookOpen size={12} className="text-[var(--green)]" />
            The Fundamentals — 16 Core Christian Truths
          </div>
